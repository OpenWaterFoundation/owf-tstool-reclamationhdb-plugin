...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Copy the TSTool Reclamation HDB (reclamationhdb) plugin installer files to the Amazon S3 static website folder:"
...
printVersion() {
  ...
  echoStderr "Copyright 2025 Open Water Foundation."
  echoStderr ""
...
uploadInstaller() {
  ...
  installerUploadCount=0
    ...
    if [ "$includeWindows" = "yes" ]; then
      ...
      zipFilePattern="${distFolder}/tstool-reclamationhdb-plugin-${pluginVersion}-win-*.zip"
      ...
      if [ -f "${latestZipFile}" ]; then
        ...
        if [ ${errorCode} -ne 0 ]; then
          ...
          cloudFrontFile="/tstool-reclamationhdb-plugin/${pluginVersion}/software/*"
...
logInfo ""
logInfo "${installerUpdateCount} TSTool Reclamation HDB plugin installers were uploaded to Amazon S3 location:"
logInfo ""
logInfo "  ${s3FolderUrl}"
logInfo "  ${gpDownloadUrl}"
logInfo ""
logInfo "Next steps are to do the following:"
logInfo "  - Visit the following folder to download the TSTool Reclamation HDB plugin:"
logInfo "      ${gpDownloadUrl}/"
logInfo ""
